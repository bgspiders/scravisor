...
class RedisMixin:
    ...
    def next_requests(self):
        ...
        found = 0
        while True:
            # if found >= self.redis_batch_size:
            #     break
    ...
    def make_request_from_data(self, data):
        ...
        """
        try:
            olg_req = bytes_to_str(data, self.redis_encoding)
            meta = olg_req['meta']
            if olg_req.get('method', 'GET').upper() == 'POST':
                data = meta.get('data', {})
                meta['data'] = data
                meta['__type'] = meta.get('__type')
                req = JsonRequest(
                    url=olg_req['url'].strip(),
